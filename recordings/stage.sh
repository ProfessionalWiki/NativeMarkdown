...
#   ./stage.sh baseline    # dogfood "before" state (with the leading H1)
#   ./stage.sh demo        # apply the demo edit as a deterministic stand-in (used for stills)
#   ./stage.sh seed        # full dogfood seed, fresh wiki only
...
exec docker compose exec -T mediawiki \
  bash /var/www/html/extensions/NativeMarkdown/recordings/container-stage.sh "${1:-}"
